...
public class Comm01ServiceImpl implements Comm01Service {
    @Value("${ccbs.userInfoApiUrl}")
    private String userInfoApiUrl;

    @Value("${ccbs.userInfoApiKey}")
    private String userInfoApiKey;

    @Value("${ccbs.userInfoApiIv}")
    private String userInfoApiIv;
    
    @Autowired
    private CommOfficeService commOfficeService;
...
//        inputStr = "D600552890";
    ...
    private static int getEndCharCheckValue(String inputStr) {
        ...
        int charSum = char1*1+char2*8+char3*7+char4*6+char5*5+char6*4+char7*3+char8*2+char9*1;
...
//3 	地址 	maskAddress 	string 	256 	地址(顯示至路名或12Bytes) 惟『路』前面之一個中文字， 必須隱碼
//4 	銀行帳號或信用卡號碼 	maskBankAccount 	string 	25 	銀行帳號或信用卡號碼(需隱碼倒數2,4,6,8碼)
    ...
    public static PersonalInfoMaskStr COMM01_0002(PersonalInfoMaskStr inputStr) {
        ...
        if(!inputStr.isMaskFirstNameNull()){
            ...
            if (isChinese) {
                ...
                String outputMaskFirstNameCht = firstChar + inputStr.getMaskFirstName().substring(1).replaceAll(regex, replacement);
                ...
                Character lastChar = inputStr.getMaskFirstName().toCharArray()[inputStr.getMaskFirstName().length() - 1];

                String outputMaskFirstNameEng = firstChar + inputStr.getMaskFirstName().substring(1, inputStr.getMaskFirstName().length() - 1).replaceAll(regex, replacement) + lastChar;
...
//                // Replacement string
//                String replacement = "*";
//                String outputMaskIDNumber = inputStr.getMaskIDNumber().substring(6).replaceAll(regex, replacement);
...
//                       //台臺 台臺
            ...
            if (inputStr.getMaskAddress().indexOf("路") != -1) {
                ...
                String whiteCircle = "\u25CB";
                String outputMaskAddress = inputStr.getMaskAddress().substring(0, roadIdx - 1) + whiteCircle + "路";
    ...
    private static boolean isChineseCharacter(char ch) {
        ...
        //comment out tmp
//                        (ch >= '\u20000' && ch <= '\u2A6DF') || // CJK Unified Ideographs Extension B
//                        (ch >= '\u2A700' && ch <= '\u2B73F') || // CJK Unified Ideographs Extension C
//                        (ch >= '\u2B740' && ch <= '\u2B81F') || // CJK Unified Ideographs Extension D
//                        (ch >= '\u2F800' && ch <= '\u2FA1F'); // CJK Compatibility Ideographs Supplement
    }
    ...
    public UserInfoQueryOut COMM001_0005(UserInfoQueryIn userInfoQueryIn) {
        ...
        try {
            ...
            userInfoQueryApiIn.setEMP_ID(userInfoQueryIn.getEmpId().substring(userInfoQueryIn.getEmpId().length() - 6));
            ...
            ResponseEntity<UserInfoQueryOut> response = restTemplate.exchange(url, HttpMethod.POST, requestEntity, UserInfoQueryOut.class);
            log.debug("after post api request, response: ",response);
            log.debug("after post api request, response headers: "+response.getHeaders());
            log.debug("after post api request, response statusCodeValue: "+response.getStatusCodeValue());
...
//        String plaintext = "{\"SYSTEM_ID\":\"BP\",\"TAX_ID\":\"A\",\"QUERY_TYPE\":\"1\",\"ITEM_CODE_LIST\":null}";
...
//            random.nextBytes(ivBytes);
    ...
    public static <T> T post(String callApiUrl, Object queryEntity, Class<T> returnClass) throws Exception{
...
//            _log.info("[call apiUrl[" + callApiUrl + "] input[" + mapper.writeValueAsString(queryEntity) + "] return[" + sbReadline.toString() + "]");
            if (returnClass != null) {
...
//            _log.error("call apiUrl[" + callApiUrl + "] error", e);
    ...
    private static HttpsURLConnection postSSL(String callApiUrl, Object queryEntity) throws Exception {
...
//            _log.error("", e);
    ...
    public OfficeInfoQueryOut COMM01_0003(OfficeInfoQueryIn inputStr) {
        ...
        try {
            ...
            Optional<CommOffice> returnCommOffice = Optional.ofNullable(commOfficeService.getAllRelatedOffices(officeCode, transType));
