...
public class RptLogServiceImpl implements RptLogService {
  @Autowired private RptLogsMapper rptLogsMapper;

  @Autowired private RptListMapper rptListMapper;

  @Autowired private RptLogsDetlsMapper rptLogsDetlsMapper;
  ...
  RptLogsDetlsDynamicSqlSupport.RptLogsDetls rptLogsDetls =
      RptLogsDetlsDynamicSqlSupport.rptLogsDetls.withAlias("rptLogsDetls");
    ...
    try {
      ...
      if (input.getIsRerun().equals("Y")) {
        for (int i = 0; i < input.getDDataSet().size(); i++) {
          ...
          UpdateStatementProvider provider =
              update(rptLogsDetls)
                  .set(rptLogsDetls.rptLogsStatus)
                  .equalTo("E")
                  .where(rptLogsDetls.rptFileName,
                      isEqualTo(input.getDDataSet().get(i).getRptFileName()))
      ...
      if (updateFlg) {
        for (int i = 0; i < input.getDDataSet().size(); i++) {
          ...
          rptLogsDetls.setRptFileCount(
              new BigDecimal(input.getDDataSet().get(i).getRptFileCount()));
  ...
  public List<BatchRptQueryOut> batchRptQuery(BatchRptQueryIn input) {
    ...
    List<String> rptCodeListByFunCodeStr = null;

    // get rpt code list from fun code
    if (input.getRptCode() != null) {
      rptCodeListByFunCodeStr = List.of(input.getRptCode());
    } else if (input.getRptCategory() != null) {
      ...
      rptCodeListByFunCodeStr = new ArrayList<>();
    ...
    if (!input.isRrptSecretMarkNull() && input.getRptSecretMark().equals("N")) {
      SelectStatementProvider rptLogsDetlsSecretMarkProvider =
          select(rptLogsDetls.withAlias("detls").allColumns())
              .from(rptLogsDetls.withAlias("detls"))
              .where(rptLogsDetls.withAlias("detls").rptDate,
                  isGreaterThanOrEqualToWhenPresent(input.getRptDateStart()))
              .and(rptLogsDetls.withAlias("detls").rptDate,
                  isLessThanOrEqualToWhenPresent(input.getRptDateEnd()))
              .and(
                  rptLogsDetls.withAlias("detls").billOff, isEqualToWhenPresent(input.getBillOff()))
              .and(rptLogsDetls.withAlias("detls").rptTimes,
                  isEqualToWhenPresent(input.getRptTimes()))
              .and(rptLogsDetls.withAlias("detls").rptSecretMark,
                  isEqualTo(input.getRptSecretMark()))
              .and(rptLogsDetls.withAlias("detls").billCycle,
                  isEqualToWhenPresent(input.getRptPeriod()))
              .and(rptLogsDetls.withAlias("detls").billMonth,
                  isEqualToWhenPresent(input.getRptYearMonth()))
              .and(exists(select(rptLogs.withAlias("logs").allColumns())
                              .from(rptLogs.withAlias("logs"))
                              .where(rptLogs.withAlias("logs").rptLogsId,
                                  isEqualTo(rptLogsDetls.withAlias("detls").rptLogsId))
                              .and(rptLogs.withAlias("logs").rptCode,
                                  isInWhenPresent(rptCodeListByFunCodeStr))))
      ...
      SelectStatementProvider rptLogsDetlsProvider =
          select(rptLogsDetls.withAlias("detls").allColumns())
              .from(rptLogsDetls.withAlias("detls"))
              .where(rptLogsDetls.withAlias("detls").rptDate,
                  isGreaterThanOrEqualToWhenPresent(input.getRptDateStart()))
  ...
  public List<RptCategoryOut> getRptCodeOptions(String funCode) {
    ...
    if (funCode != null) {
        rptListProvider = select(rptList.allColumns())
            .from(rptList)
    ...
    for (int i = 0; i < rptListOut.size(); i++) {
      ...
      rptCategoryOut.setName(rptListOut.get(i).getRptCode().concat(" ").concat(rptListOut.get(i).getRptName()));
  ...
  public List<RptList> getRptCodeByFunCode(String funCode) {
    // query rpt code
    SelectStatementProvider rptListProvider = select(rptList.rptCode)
                                                  .from(rptList)
                                                  .where(rptList.funCode, isEqualTo(funCode))
                                                  .build()
                                                  .render(RenderingStrategies.MYBATIS3);
  ...
  public List<RptCategoryOut> getFunCodeOptions() {
    // query RPT_LIST
    SelectStatementProvider rptListProvider =
        selectDistinct(rptList.funCode).from(rptList).build().render(RenderingStrategies.MYBATIS3);
    List<RptList> rptListOut = rptListMapper.selectMany(rptListProvider);
